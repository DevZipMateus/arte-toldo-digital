...
const Products = () => {
  ...
  const categories = [
    { id: 'fixos', label: 'Toldos Fixos', icon: <Home className="w-4 h-4" /> },
    { id: 'retrateis', label: 'Toldos Retráteis', icon: <Zap className="w-4 h-4" /> }
  ];
  ...
  const MaterialBadge = ({ material }: { material: string }) => {
    ...
    return (
      <Badge variant={getVariant(material)} className="text-xs">
        {material}
      </Badge>
    );
  };
    ...
    <section id="produtos" className="py-20 bg-white">
      <div className="container mx-auto px-4">
        ...
        <Tabs value={selectedCategory} onValueChange={setSelectedCategory} className="w-full">
          ...
          {Object.entries(productTypes).map(([categoryId, products]) => (
            <TabsContent key={categoryId} value={categoryId}>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {products.map((product, index) => (
                  <Card 
                    key={index} 
                    className={`p-6 hover:shadow-xl transition-all duration-300 hover:transform hover:scale-105 border-0 arte-shadow-soft ${
                      product.highlight ? 'ring-2 ring-arte-blue-light' : ''
                    }`}
                  >
                    {product.highlight && (
        ...
        <div className="mt-16 text-center bg-arte-gradient-primary rounded-2xl p-12 text-white">
          ...
          <Button 
            size="lg"
            className="bg-white text-arte-blue-royal hover:bg-blue-50 font-semibold px-8 py-4"
            onClick={scrollToContact}
          >
            Falar com Consultor
          </Button>
